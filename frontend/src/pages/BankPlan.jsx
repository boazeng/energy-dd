...
const FIG = Object.fromEntries([
  'investment',       // 1.3  תכנית ההשקעות
  'summary',          // 1.4  תמצית התחזית הכספית
  'pnlForecast',      // 1.5  רווח והפסד צפוי
  'profitChart',      // 1.6  תזרים נטו על פני השנים
  'sensitivity',      // 1.7  רגישות התוצאות לקצב החדירה
...
/* ─── טבלה 1 — תכנית ההשקעות ──────────────────────────────────────────────
   ...
   בפסקה שמעל הטבלה, כך שרצועת מקורות בת שורה אחת רק חזרה עליו. הון עצמי,
   ככל שיידרש, מופיע בהערה שמתחת. */
...
function SummaryCompact({ d }) {
  ...
  const rows = [
    ...
    // ואינן מנוכות מהתזרים. מקומן בטבלת תכנית ההשקעות שבסעיף 1.3.
    ...
    // קרן. הרווח וההפסד — ריבית בלבד ומס חברות — מוצג בטבלה שבסעיף 1.5.
...
/* ─── טבלה 7 — נתוני רווח והפסד ────────────────────────────────────────────
   ...
   ב-2025 נכללה בספרים עסקת דניה סיבוס, שאינה חלק מהפעילות הנרכשת. היא נרשמה
   בהכנסה ובהוצאה באותו סכום, ולכן הנטרול אינו משנה את השורה התחתונה. */
...
function SummaryFinancials({ d }) {
  ...
  const rows = [
    ...
    // ההצגה החשבונאית — ריבית בלבד ומס חברות — מרוכזת בסעיף 1.5.
  ]
  return (
    <figure className="bkp-figure">
        ...
        <thead>
          <tr>
            ...
            {/* התוצאה החשבונאית לצד התזרים — ריבית בלבד ובניכוי מס חברות.
                פירוט הדרך אליה, לרבות סכום הריבית והמס, מופיע בטבלה שבסעיף 1.5;
...
מאפיין מרכזי של העסקה הוא שההכנסה מגיעה מרגע ההשלמה ואינה מותנית בתקופת הרצה — ולפיכך שירות החוב נשען על תזרים קיים ולא על תחזית בלבד.`,
  ...
  // הרווח וההפסד (סעיף 1.5) — מזהה עצמאי, כמו שאר הבלוקים שאינם מספר סעיף
...
הגבייה מתבצעת דיגיטלית, והתחזוקה והניטור מנוהלים מרחוק.`,
  ...
  'pnl-danya': `בשנת 2025 נכללה בספרי החברה עסקה מול דניה סיבוס בסך 2,575,255.54 ₪ כולל מע"מ — 2,182,419.95 ₪ בנטרול מע"מ בשיעור 18%. העסקה אינה חלק מהפעילות הנרכשת ואינה צפויה לחזור, ולכן הוצגה לצדה גם שנת 2025 בנטרולה.

הסכום נרשם בהכנסות ובעלות ההכנסות כאחד, ומכאן ששני הצדדים מתקזזים: הנטרול מקטין את מחזור ההכנסות מ-3,075,985 ₪ ל-893,565 ₪ ואת עלות ההכנסות מ-2,554,573 ₪ ל-372,153 ₪, ואינו משנה את הרווח לשנה, שנותר 378,262 ₪. הטור המנוטרל הוא הבסיס הרלוונטי להשוואה מול 2024 ומול המחצית הראשונה של 2026.`,
  ...
  // ניתוח הרגישות עבר לסעיף 1.5. סעיף 7.5 (יחסי כיסוי החוב) הוסר מגוף המסמך
...
const TOC = [
  ...
  ['1.2', 'בקשת האשראי', 2],
  ['1.3', 'תכנית ההשקעות', 2],
  ['1.4', 'תמצית התחזית הכספית', 2],
  ['1.5', 'רווח והפסד צפוי', 2],
  ['1.6', 'רווחיות לאורך התקופה', 2],
  ['1.7', 'ניתוח רגישות', 2],
  ['1.8', 'יתרונות הרכישה', 2],
  ['1.9', 'הבהרות', 2],
    ...
    <div className="bkp-page">
      ...
      <article className="bkp-sheet" dir="rtl">
        ...
        <section className="bkp-section bkp-level-1">
          ...
          {/* תכנית ההשקעות היא סעיף בפני עצמו ולא רצועת "שימושים" בתוך בקשת
              האשראי: הפסקה קובעת את הסכום המבוקש, והטבלה שאחריה מפרטת לאן הוא
              הולך ומסתכמת בו בחזרה. */}
          <H2 n="1.3">תכנית ההשקעות</H2>
          <p className="bkp-p">
            האשראי המבוקש עומד על <strong>{ils(d.acquisition.credit_requested)}</strong>.
            להלן תכנית ההשקעות שלמימונה הוא מיועד:
          </p>
          <InvestmentPlanTable d={d} />

          <H2 n="1.4">תמצית התחזית הכספית</H2>
          <P id="1.3" />
          <SummaryCompact d={d} />
